...
//                groceryList.getQty(), groceryList.getUserId()) == 1;
    ...
    public boolean addNewItemToGroceryList(long id, GroceryList groceryList) {
        String sql = "INSERT INTO grocery_list ( ingredient_id, ingredient_name, qty, user_id, complete) " +
                 " SELECT i.id, ?, ?, ?, false FROM ingredients AS i WHERE i.name = ? LIMIT 1";
        return jdbcTemplate.update(sql,groceryList.getingredient_name(), groceryList.getQty(), groceryList.getUserId(), groceryList.getingredient_name()) == 1;
    }
